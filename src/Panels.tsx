...
export function Sheet({ children, close, title, hidden = false }: { children: ReactNode; close: ()=>void; title: string; hidden?: boolean }) {
  ...
  useEffect(()=>{
    ...
    const onKey=(event: KeyboardEvent)=>{
      ...
      if(event.key==='Tab') {
        const focusable=Array.from(ref.current?.querySelectorAll<HTMLElement>('button:not(:disabled),input:not(:disabled),select:not(:disabled),textarea:not(:disabled),a[href],summary,iframe,[tabindex="0"]') ?? []).filter(element => element.getClientRects().length > 0 && !element.closest('[inert]'))
  ...
  return <div className="modal-backdrop" hidden={hidden} onClick={close}><div className="sheet glass" role="dialog" aria-modal="true" aria-label={title} tabIndex={-1} ref={ref} onClick={e=>e.stopPropagation()}><div className="sheet-grabber"/><header className="sheet-header"><span>{title}</span><button className="circle-button" onClick={close} aria-label="关闭"><Icon name="close" size={18}/></button></header><div className="sheet-body" key={title}>{children}</div></div></div>
}
...
export function GamePanel({ variant, complete, close }: { variant:number;complete:(id:string)=>void;close:()=>void }) {
  ...
  if(phase==='intro') return <><div className="game-intro-art">{variant===0?<JourneyArtwork/>:<Icon name="book" size={70}/>}</div><span className="eyebrow">{variant===0?'串联路线':'历史回顾'}</span><h2 className="panel-title">{variant===0?'把足迹，连成征途':'五问，走近长征'}</h2><p className="panel-description">{variant===0?'按时间先后，依次选择长征中的七个节点。每一次选择，都让这段历史更清晰。':'从五个历史问题出发，了解长征的重要地点与时刻。每一题都有简短解读。'}</p><div className="route-facts"><span><Icon name="clock" size={16}/>约 2 分钟</span><span><Icon name="profile" size={16}/>单人探索</span></div><Action onClick={()=>setPhase('play')}>开始探索</Action></>
  if(phase==='result') return <div className="game-result"><span className="result-mark"><Icon name="check" size={44}/></span><p className="eyebrow">再走近一点</p><h2>又走近历史一点。</h2><p>{variant===0?'你已串联起七个重要节点。':`你答对了 ${score} / ${questions.length} 题，完成本次学习。`}</p><div className="result-note"><strong>{variant===0?'从出发，到会师':'从历史，到今天'}</strong><p>真正留下的，不只是一个答案，<br/>还有继续了解的好奇心。</p></div><Action icon="check" onClick={close}>记住这一刻</Action></div>
  if(variant===0) return <><div className="quiz-heading"><span>路线探索</span><span>{order.length} / {exhibits.length}</span></div><div className="thin-progress"><i style={{width:`${order.length / exhibits.length * 100}%`}}/></div><h2 className="quiz-question">{order.length===exhibits.length?'每个节点，都已连接。':'下一站，在哪里？'}</h2><p className="panel-description">从 1934 年出发，按时间顺序点亮路线。</p><div className="order-trail">{exhibits.map((e,i)=><span className={order.includes(i)?'lit':''} key={e.id}>{order.includes(i)?<Icon name="check" size={15}/>:i+1}</span>)}</div><div className="answer-list">{[3,1,6,4,0,5,2].map(i=><button disabled={order.includes(i)} key={i} onClick={()=>{if(i===order.length){setOrder([...order,i]);setIncorrect(false)}else setIncorrect(true)}}><span>{exhibits[i].name}</span>{order.includes(i)?<Icon name="check" size={18}/>:<Icon name="chevron" size={13}/>}</button>)}</div><div className="answer-feedback" aria-live="polite">{incorrect?'再想一想：先从江西出发，之后在遵义迎来转折。':order.length>0?`${exhibits[order[order.length-1]].date} · ${exhibits[order[order.length-1]].place}`:'选择旅程的起点。'}</div><Action disabled={order.length!==exhibits.length} onClick={finish}>完成探索</Action></>
